...
function createRematchButton() {
  ...
  var rematchText = document.createTextNode("Lawan lagi??");
...
function createFindGameButton() {
  ...
  var findGameText = document.createTextNode("Cari Game Baru");
...
function removeSearchScreen() {
  document.getElementById("loading").setAttribute("class", "animated fadeOut");
...
function gameStart() {
  ...
  checkTurn();
  socket.emit("namapush", playerData.nama);
}
socket.on("tambah", (a) => {
...
socket.on("overCap", function (roomId) {
  document.getElementById("searchState").innerHTML = "Room " + roomId + " sudah penuh";
});

//Runs when other player disconnected
socket.on("playerDisconnect", function () {
  playerDisconnected("Lawan terputus");
});

function restartGame() {
  document.getElementById("rematchButton").remove();
  sandwichWithSpinners("gameState", " Menunggu Lawan ");
...
socket.on("winnerDetermined", function (winner) {
  if (winner.youWon) {
    document.getElementById("gameState").innerHTML = "Kamu menang!";
  } else {
    document.getElementById("gameState").innerHTML = "Kamu kalah...";
  }
...
socket.on("tie", function () {
  document.getElementById("gameState").innerHTML = "Seri!!!";
  endGameInit();
